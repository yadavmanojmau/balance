    ...
    while(1):
        ...
        while start[0] == 1 and end == 0:
            ...
            if end == 0:
                ...
                success, img_ = cap.read()         ### Reading frames from the camera
                # img = img_[150:420, 100:520]       ### Crop the frame.
                img = cv2.resize(img_,(frameWidth,frameHeight)) ### resize the camera output window
                ...
                imgCanny = cv2.Canny(imgGray,0,255)  ### Extracting only edges of the objects
                kernel = np.ones((5, 5))
                ...
                for cnt in contours:
                    ...
                    if 1000 < area < 8000:        ## Setting the threshold for base area

                        cv2.drawContours(img, cnt, -1, (255, 0, 255), 3)   ## Draw contours of base
                        ((x_, y_), radius) = cv2.minEnclosingCircle(c)    ### Extracting x and y coordinates of the contours of the shapes
                        ...
                        if shape == 'Circle':
                            ...
                            1, (0, 255,0), 2)
                            c.write_single_register(Shape_addr,1)
                        ...
                        ### If the detected shape is Square send the command to the robot to move accordingly
                            ...
                            1, (0, 255,0), 2)
                            c.write_single_register(Shape_addr,2)
